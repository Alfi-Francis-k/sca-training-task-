...
{{#if showDeliverySection}}
  <section class="customfields-delivery">
      <h4 class="customfields-delivery__title">Custom Delivery Details</h4>
      
      <!-- Custom Fields Displayed from getContext -->
      <p class="customfields-delivery__info">Expected Delivery: <strong>{{custitem_expected_delivery}}</strong></p>
      <p class="customfields-delivery__note">Special Note: <em>{{custcol_special_note}}</em></p>
      
      <!-- Input triggering "change" and "blur" events -->
      <div class="customfields-delivery__form-group">
        <label class="customfields-delivery__label">Delivery Note Instructions:</label>
        <input type="text" class="customfields-delivery__input" data-action="update-delivery-note" placeholder="Enter note (min 5 characters)"/>
      </div>
      
      <!-- Button triggering "click" event -->
      {{#if showWarrantyInfo}}
         <button class="customfields-delivery__button" data-action="show-warranty-info">
            View Warranty Info
         </button>
      {{/if}}
  </section>
